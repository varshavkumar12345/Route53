...
def generate_bind_zone_file(zone_name: str, records: list) -> str:
    z_name = zone_name.strip()
    if not z_name.endswith("."):
        z_name += "."
        
    lines = [
        f"; BIND Zone File Export for {z_name}",
        f"; Generated by Route53 Console replica",
        f"; Origin: {z_name}",
        "",
        "$TTL 3600",
        ""
    ...
    def sort_key(r):
        if r.type == "SOA": return 0
        if r.type == "NS": return 1
        return 2
    ...
    for r in sorted_recs:
        ...
        values = [v.strip() for v in r.value.split("\n") if v.strip()]
        for val in values:
            lines.append(f"{name:<30} {r.ttl:<6} IN  {r.type:<6} {val}")
...
def export_hosted_zone(
    ...
    if format.lower() == "bind":
        bind_text = generate_bind_zone_file(zone.name, records)
        filename = f"{zone.name.rstrip('.')}_zone.txt"
        return Response(
            content=bind_text,
        ...
        filename = f"{zone.name.rstrip('.')}_zone.json"
        return Response(
            content=json_str,
